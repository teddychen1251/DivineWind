function translateLocationToCoords(maze, mazeLocation) {
    switch (mazeLocation) {
        case BOTTOM_CENTER:
            return new MazeCoordinates(LAYERS, maze.maze[LAYERS].length / 2);
        case LEFT_CENTER:
            return new MazeCoordinates(LAYERS, 3 * maze.maze[LAYERS].length / 4);
        case RIGHT_CENTER:
            return new MazeCoordinates(LAYERS, maze.maze[LAYERS].length / 4);
        case TOP_CENTER:
...
function translateMazeCoordinatesToWorldPos(maze, mazeCoords) {
    ...
    let layerCellCount = maze.maze[mazeCoords.layer].length;
